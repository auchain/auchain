...
const (
	...
	DefaultHTTPPort = 9525        // Default TCP port for the HTTP RPC server
	DefaultWSHost   = "localhost" // Default host interface for the websocket RPC server
	DefaultWSPort   = 9526        // Default TCP port for the websocket RPC server
)
...
func DefaultDataDir() string {
	...
	if home != "" {
		if runtime.GOOS == "darwin" {
			return filepath.Join(home, "Library", "AUC")
		} else if runtime.GOOS == "windows" {
			return filepath.Join(home, "AppData", "Roaming", "AUC")
		} else {
			return filepath.Join(home, ".auc")
		}
	}
